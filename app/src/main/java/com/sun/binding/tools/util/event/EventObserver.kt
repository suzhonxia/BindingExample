package com.sun.binding.tools.util.event
...
class EventPeekObserver<T>(private val onEventUnconsumedContent: (T) -> Unit) : Observer<Event<T>> {
    override fun onChanged(event: Event<T>?) {
        event?.peek()?.run(onEventUnconsumedContent)
    }
}
